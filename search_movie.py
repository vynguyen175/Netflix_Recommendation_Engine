...
def _safe_get(url: str, params: Dict) -> Union[Dict, List]:
    """
    Make a safe GET request. If anything fails or API key is missing,
    return {} so the app won't crash.
    """
    if not API_KEY:
        return {}
...
def get_movie_details(movie_id: Union[int, str]) -> Dict:
    """
    Get full metadata for one movie.
    """
    if not movie_id:
        return {}
...
def get_recommendations(movie_id: Union[int, str]) -> List[Dict]:
    """
    Get TMDb-suggested similar/recommended titles.
    """
    if not movie_id:
        return []
...
def get_trending() -> List[Dict]:
    """
    Daily trending movies (good hero candidates).
    """
